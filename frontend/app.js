...
function renderApp() {
  ...
  root.innerHTML = `<section><h1>${esc(d.app.name)}</h1><div class="actions"><button id="new">新增紀錄</button><button id="json">匯出 JSON</button><button id="csv">匯出 CSV</button><button id="back" class="ghost">返回</button></div></section><section><h2>紀錄</h2>${
    rs.length
      ? `<div class="cards">${rs
          .map(
            (r) =>
