...
- 順手抓到「匯入 Spec」的一個真實 bug：如果貼上的 spec 的 `app.id` 跟既有 App 撞名，原本會直接無警告覆蓋掉，現在會先查一次、跳出「已經有一個 App 叫 XXX，建立後會覆蓋」的警告。
- 使用者回報「App 編輯或畫面切換不順暢」，抓到兩個真的問題：
  1. `openApp()` 原本是「先等 Definition 回來、再打 records」兩次序列請求，改成 `Promise.all` 平行打，省下一次等待。
  2. 每次存/刪一筆紀錄後，原本會呼叫整個 `openApp(current)`（連 Definition 都重新抓一次，但 Definition 根本沒變），改成新的 `refreshRecords()` 只重抓紀錄列表——最常見的「存一筆資料」動作少了一次不必要的網路來回。
  3. 載入畫面原本卡在乾巴巴的「載入中…」，現在超過 4 秒會補充說明「可能是免費方案剛醒來，第一次約需 30–60 秒」，管理使用者預期（Render 免費方案冷啟動本身沒辦法完全消除，但至少讓使用者知道發生了什麼事）。

### 實際驗證
- `npm run build` + `npm test`（17 條）、backend 對真實 Postgres 測試（6 條，含新的刪除 App cascade 測試）全部通過。
- 用 Playwright 追蹤實際網路請求時間戳，證實 `openApp()` 的兩個請求確實同時發出（不是序列），且存一筆紀錄後只剩 POST + GET records 兩個請求（原本是 3 個，包含一個不必要的 Definition 重抓）。

### 現況
準備 commit/push。使用者接下來還要求做完整 RBAC（權限分級，準備分享給同事）+ audit log（記錄編輯操作），這是下一輪更大的工作，會另外記錄。
...
- 深色模式 accent 從青綠 `#3fb3a3` 改成中性藍灰 `#6b7684`（hover `#838d99`），背景/邊框也跟著調成更中性的灰階（`--bg:#16171a`、`--surface:#1c1d20`）。整個深色模式現在沒有任何搶眼的彩色，層次靠間距跟字重區分，不是靠顏色。
- 只改深色模式；淺色模式的青綠色系維持不變（使用者沒有對淺色模式表示不滿）。

### 實際驗證
- `npm run build` + `npm test`（17 條測試）通過。
- 用 Playwright 開 `colorScheme:'dark'` 實際截圖確認：logo、按鈕、記錄數文字全部從青綠變成中性灰藍，畫面明顯更低調。
